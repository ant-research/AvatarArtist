...
import torch
from recon.models.stylegan.networks_stylegan2 import Conv2dLayer, SynthesisLayer, ToRGBLayer
from torch_utils.ops import upfirdn2d
from torch_utils import persistence
from torch_utils import misc

from recon.models.stylegan.networks_stylegan2 import SynthesisBlock
import numpy as np
from .networks_stylegan3 import SynthesisLayer as AFSynthesisLayer
...
class SuperresolutionHybrid8X(torch.nn.Module):
    ...
    def forward(self, rgb, x, ws, ws_super=None, **block_kwargs):
        if ws_super is None:
            ws = ws[:, -1:, :].repeat(1, 3, 1)
        else:
            ws = ws_super[:, -1:, :].repeat(1, 3, 1)
...
# for 512x512 patch generation, MLP-based, added by Yu Deng
@persistence.persistent_class
class SuperresolutionPatchMLP(torch.nn.Module):
    def __init__(self, channels, img_resolution, sr_num_fp16_res, sr_antialias,
                num_fp16_res=4, conv_clamp=None, channel_base=None, channel_max=None,# IGNORE
                **block_kwargs):
        super().__init__()
        ...
        self.block0 = SynthesisBlockNoUp(channels, 128, w_dim=512, kernel_size=1, resolution=256, use_noise=False,
                img_channels=3, is_last=False, use_fp16=use_fp16, conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        self.block1 = SynthesisBlock(128, 64, w_dim=512, kernel_size=1, resolution=512, use_noise=False,
                img_channels=3, is_last=True, use_fp16=use_fp16, conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        self.register_buffer('resample_filter', upfirdn2d.setup_filter([1,3,3,1]))
...
class SuperresolutionHybrid8XDC(torch.nn.Module):
    def __init__(self, channels, img_resolution, sr_num_fp16_res, sr_antialias,
                num_fp16_res=4, conv_clamp=None, channel_base=None, channel_max=None, use_ws_ones=False,# IGNORE
                **block_kwargs):
        super().__init__()
        assert img_resolution == 512

        use_fp16 = sr_num_fp16_res > 0
        self.use_ws_ones = use_ws_ones
        self.input_resolution = 128
        self.sr_antialias = sr_antialias
