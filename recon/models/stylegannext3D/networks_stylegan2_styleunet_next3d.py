...
class EncoderResBlock(torch.nn.Module):
    def __init__(self, img_channel, in_channel, out_channel, down, downsample, resample_filter=[1, 3, 3, 1]):
        super().__init__()
        ...
        self.conv2 = Conv2dLayer(in_channel, out_channel, 3, down=down, activation='lrelu')
    ...
    def forward(self, input, skip=None):
        ...
        if self.downsample != 1:
            x = upfirdn2d.downsample2d(x,  self.resample_filter, down=self.downsample)
        out = self.fromrgb(x)
        if skip is not None:
            out = out + skip
        out = self.conv1(out)
