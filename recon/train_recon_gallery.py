...
import dnnlib
from training import training_loop_recon_gallery as training_loop
...
def init_dataset_kwargs(opts, class_name='training.dataloader.dataset_gallery.ImageFolderDataset'):
    try:
        dataset_kwargs = dnnlib.EasyDict(class_name=class_name,
                                         path=opts.data, data_label_path=opts.data_label_path,
                                         label_file_vfhq=opts.label_file_vfhq, label_file_ffhq=opts.label_file_ffhq,
                                         mesh_path_ffhq = opts.mesh_path_ffhq,  motion_path_ffhq=opts.motion_path_ffhq, mesh_path_vfhq=opts.mesh_path_vfhq, motion_path_vfhq=opts.motion_path_vfhq,
...
def main(**kwargs):
    ...
    c.G_kwargs = dnnlib.EasyDict(class_name=None, mot_dims = 548)

    c.D_kwargs = dnnlib.EasyDict(class_name=None,
                                 block_kwargs=dnnlib.EasyDict(), mapping_kwargs=dnnlib.EasyDict(),
                                 epilogue_kwargs=dnnlib.EasyDict())
    c.D_patch_kwargs = dnnlib.EasyDict(class_name=None, block_kwargs=dnnlib.EasyDict(),
                                       epilogue_kwargs=dnnlib.EasyDict())
    ...
    c.loss_kwargs = dnnlib.EasyDict(class_name='training.loss.loss_recon_gallery.AnimatablePortraitReconLoss')
    c.data_loader_kwargs = dnnlib.EasyDict()

    # Training set.

    c.training_set_kwargs, dataset_name = init_dataset_kwargs(opts)
    # c.training_set_kwargs = dnnlib.EasyDict(class_name='online', resolution=512, data_type=None, path='',
    #                                         use_labels=True, max_size=None, xflip=False)
    # dataset_name = 'online'
    # if opts.static:
    #     dataset_name += '-static'
    ...
    # c.training_set_kwargs.motion_params_path1 = opts.motion_params_path1 # check what is this
    ...
    c.loss_kwargs.r1_gamma_seg = opts.gamma_seg # check what is this
    c.loss_kwargs.r1_gamma_patch = opts.gamma_patch# check what is this
    c.loss_kwargs.truncation_psi = opts.truncation_psi # check what is this
    c.G_opt_kwargs.lr = opts.glr
    c.cross_lr_scale = opts.cross_lr_scale  # check what is this
    ...
    c.render_pretrain = opts.render_pretrain

    c.gan_model_base_dir = opts.gan_model_base_dir
    c.vae_pretrained = opts.vae_pretrained
    ...
    c.loss_kwargs.gmain = 0.01 # check what is this
    ...
    c.G_kwargs.sr_kwargs = dnnlib.EasyDict(channel_base=opts.cbase, channel_max=opts.cmax,
